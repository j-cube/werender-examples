...
def main(session):
    # log file
    wr.set_log_file(os.path.join(tempfile.gettempdir(), 'displacement.log'))
    ...
    tex_disp = scene.new_texture(wr.textures.File)
    tex_disp.set_colorspace("linear")
    tex_disp.set_tonal_adjust(1)
    ...
    settings.set_image_name('displacement.jpg')
    ...
    result.download_image(tempfile.gettempdir(), 'displacement.jpg')
