...
def main(session):
    # log file
    wr.set_log_file(os.path.join(tempfile.gettempdir(), 'bump.log'))
    ...
    tex_normal_asset = session.upload(resolve_file('rock_clifflayer_bumpdisp_rgb.tif'), remote_folder)
    ...
    # when rendering with an approximation like bump it is best to render as
    # un-smoothed raw polygons, hence we set it to off. This is already the case
    # as the asset is a polygon and not a subdivision surface, but if it was we
    # should set:
    ...
    settings.set_image_name('bump.jpg')
    ...
    result.download_image(tempfile.gettempdir(), 'bump.jpg')
